...
from neural_methods.model.HRMamba import Vim
from neural_methods.trainer.BaseTrainer import BaseTrainer
from neural_methods.loss.TorchLossComputer import HRMamba_Loss


class HRMambaFormerTrainer(BaseTrainer):
    ...
    def __init__(self, config, data_loader):
        ...
        if config.TOOLBOX_MODE == "train_and_test":
            ...
            raise ValueError("HRMamba trainer initialized in incorrect toolbox mode!")
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                pred_ppg = (pred_ppg - torch.mean(pred_ppg, axis=-1).view(-1, 1)) / torch.std(pred_ppg, axis=-1).view(
                    -1, 1)  # normalize

                loss = 0.0
                for ib in range(N):
                    loss = loss + self.criterion(pred_ppg[ib], labels[ib], epoch, self.config.TRAIN.DATA.FS,
                                                 self.diff_flag)
                loss = loss / N
                loss.backward()
                self.optimizer.step()
    ...
    def valid(self, data_loader):
        ...
        with torch.no_grad():
            ...
            for valid_idx, valid_batch in enumerate(vbar):
                ...
                pred_ppg_valid = (pred_ppg_valid - torch.mean(pred_ppg_valid, axis=-1).view(-1, 1)) / torch.std(
                    pred_ppg_valid, axis=-1).view(-1, 1)  # normalize
                for ib in range(N):
                    loss = self.criterion(pred_ppg_valid[ib], labels_valid[ib], self.config.TRAIN.EPOCHS,
    ...
    def save_model(self, index):
        self.model_dir = os.path.join(self.model_path,self.model_dir)
